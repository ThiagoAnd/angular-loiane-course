...
export class CursosFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.formulario.valid) {
      let msgSucesso = "Sucesso ao criar curso";
      let msgErro = "Erro ao criar curso : ";
      if (this.formulario.value.id) {
         msgSucesso = "Sucesso ao atualizar curso";
      ...
      this.service.save(this.formulario.value).subscribe(
        ...
        (erro) => alert(msgErro + erro)
      );
      /* Comentado para usar a forma de cima que é mais enxuta e não duplica codigo
      if (this.formulario.value.id) {
        //vai atualizar curso se tiver um id, ou seja se tiver na rota de atualizar
        //No formulario.value vai ter tanto o id quanto o nome
        this.service.update(this.formulario.value).subscribe(
          (sucesso) => {
            alert('Sucesso ao atualizar curso');
            this.location.back();
          },
          (erro) => alert('Erro ao atualizar curso: ' + erro),
  ...
  hasErrors(campo: string) {
    ...
    console.log(
      'Campo required: ' + this.formulario.controls[campo].errors?.required
    );
